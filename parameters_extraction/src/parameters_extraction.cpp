...
//Get Model -> From URDF can obtain rbdl model, joint names, joint pos min and max, joint vel min and max, ...
 ...
 // RigidBodyDynamics::ModelDatad model_data_ = *rbdl_model_.getModelData();
  ...
  //Eigen::Vector3d temp_l_ = {ee_l_.position.x, ee_l_.position.y, ee_l_.position.z};
  //Eigen::Vector3d temp_r_ = {ee_r_.position.x, ee_r_.position.y, ee_r_.position.z};


  //Jacobians computation
  RigidBodyDynamics::CalcPointJacobian(arm_left_model_, q_l_, 7, arm_left_model_.mBodies[7].mCenterOfMass, J_l_, true);
  RigidBodyDynamics::CalcPointJacobian(arm_right_model_, q_r_, 7, arm_right_model_.mBodies[7].mCenterOfMass, J_r_, true);

  RigidBodyDynamics::CalcBodySpatialJacobian(arm_left_model_, *arm_left_model_.getModelData(), q_l_, 7, G_l_, true);
  RigidBodyDynamics::CalcBodySpatialJacobian(arm_right_model_, *arm_right_model_.getModelData(), q_r_, 7, G_r_, true);

  //Inertia computation
  RigidBodyDynamics::CompositeRigidBodyAlgorithm (arm_left_model_, q_l_, H_l_, true);
  RigidBodyDynamics::CompositeRigidBodyAlgorithm (arm_right_model_, q_r_, H_r_, true);

  //Non linear term computation
  RigidBodyDynamics::NonlinearEffects (arm_left_model_, *rbdl_model_.getModelData(),q_l_, qdot_l_, nonlin_l_);
  RigidBodyDynamics::NonlinearEffects (arm_right_model_, *rbdl_model_.getModelData(),q_r_, qdot_r_, nonlin_r_);
  ...
  Eigen::VectorXd pose_l_;
  pose_l_.resize(6);
  pose_l_.setZero();
  
  Eigen::VectorXd pose_r_;
  pose_r_.resize(6);
  pose_r_.setZero();
  ...
  pose_l_ = (J_l_*qdot_l_);
  pose_l_ += pose_l_*dt;
  cout<<" Computed left ee pose: "<<endl; 
  cout<<" "<<pose_l_<<endl<<endl;

  //Comparision (point Jacobian, just for the position I suppose)
  cout<<"Original left ee pose: "<<endl;
  cout<<" "<<ee_l_<<endl<<endl;


  //Right end effector pose computation 
  pose_r_ = (J_r_*qdot_l_);
  pose_r_ += pose_r_*dt;
  cout<<" Computed right ee pose: "<<endl;
  cout<<" "<<pose_r_<<endl<<endl;

  //Comparision (point Jacobian, just for the position I suppose)
  cout<<"Original right ee pose: "<<endl;
  cout<<" "<<ee_r_<<endl<<endl;
